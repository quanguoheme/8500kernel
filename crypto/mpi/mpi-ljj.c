...
static unsigned long long private_div(unsigned long long a,unsigned long long b)
{
	return a/b;
}
...
void mpi_div_int32(pmpi_t dst, pmpi_t a, unsigned long b)
{
    ...
    mpi_mov(dst, *a);
    if(dst->m_nLength==1){dst->m_ulValue[0]=private_div(dst->m_ulValue[0],b);return;}
    for(i=dst->m_nLength-1;i>=0;i--)
    {
        div=carry;
        div=(div<<32)+dst->m_ulValue[i];
        dst->m_ulValue[i]=private_div(div,b);
        mul=private_div(div,b)*b;
...
static unsigned long long private_mod(unsigned long a,unsigned long b)
{
	return a%b;
}
...
void mpi_mod_int32(pmpi_t dst, pmpi_t a, unsigned long b)
{
    ...
    if(a->m_nLength==1){
		mpi_mov_int64(dst, private_mod(a->m_ulValue[0],b));
		return;
	}
