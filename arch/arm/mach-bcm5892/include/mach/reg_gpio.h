...
typedef enum 
{
	...
	//e-->10
	BCM5892_GPE0,  BCM5892_GPE1,  BCM5892_GPE2,  BCM5892_GPE3, BCM5892_GPE4,  BCM5892_GPE5,  BCM5892_GPE6,
...
#define REGOFFSET_GPIO_PAD_RES      0x034 /* GPIO IO Programmable resistor direction */  //设置上拉下拉
#define REGOFFSET_GPIO_RES_EN       0x038 /* GPIO IO Programmable resister enable */  	 //上拉下拉电阻使能
...
#define REGOFFSET_GPIO_SLEW_SW      0x05C /* This register provides the GPIO IO pad slew controllability. Default is 1抌1. */
...
static inline void reg_gpio_iotr_set_pin_type( int pin, GPIO_PIN_TYPE pinType )
{
	...
	//由于GPIO口是连号设计的(0~113),那么此操作就是把GPIO号,转化成具体的一组中的号码
...
#if 1
//设置沿触发方式:上升沿或者下降沿,或者双沿触发，高低电平触发
//此函数需配合reg_gpio_itr_set_interrupt_type_level()函数使用
static inline void reg_gpio_itr_set_interrupt_type( int pin, GPIO_INTERRUPT_TYPE interruptType )
{
   ...
   HW_IRQ_SAVE( Gpio, flags );
		   ...
		   {
				   REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_INT_DE) &= ~nBitMask;   //选择使用单边触发方式
				   
				   if((interruptType==GPIO_LOW_LEVEL_INTERRUPT_TRIGGER)||(interruptType==GPIO_FALLING_EDGE_INTERRUPT_TRIGGER))
				   	{
				   			REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_INT_EDGE) &= ~nBitMask;  //下降沿触发或者低电平触发
				   	}
				   else if((interruptType==GPIO_HIGH_LEVEL_INTERRUPT_TRIGGER)||(interruptType==GPIO_RISING_EDGE_INTERRUPT_TRIGGER))
				   	{
				   			REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_INT_EDGE) |= nBitMask;   //上升沿触发或者高电平触发
				   	}
		   	}			   
			else if(interruptType==GPIO_BOTH_EDGE_INTERRUPT_TRIGGER)	//双边触发
			{
...
//added by lee  设置中断类型，是edge触发方式还是level触发方式
static inline void reg_gpio_itr_set_interrupt_type_level( int pin, GPIO_INTERRUPT_TYPE interruptType )
{
   ...
   HW_IRQ_SAVE( Gpio, flags );
   if(interruptType==GPIO_LEWEL_TRIGGER)
   		REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_INT_TYPE) |= nBitMask; /* TODO: INTERRUPT_TYPE */
   else if(interruptType==GPIO_EDGE_TRIGGER)
   		REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_INT_TYPE) &= ~nBitMask;
   HW_IRQ_RESTORE( Gpio, flags );
}
