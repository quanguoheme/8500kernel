...
#define REGOFFSET_GPIO_RES_EN       0x038 /* GPIO IO Programmable resister enable */  	 //����������ʹ��
...
static inline void reg_gpio_iotr_set_pin_type( int pin, GPIO_PIN_TYPE pinType )
{
	...
	//����GPIO����������Ƶ�(0~113),��ô�˲������ǰ�GPIO��,ת���ɾ�����һ���еĺ���
    ...
    switch(pinType)
    {
        ...
        case GPIO_PIN_TYPE_OUTPUT:
             ...
             {
                 REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_EN) |= nBitMask;  //����Ϊ������λ��1
			 }
			 else
			 {
                 REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_EN) &= ~nBitMask; //����Ϊ������λ��0
			 }
...
{
    unsigned long flags;
    unsigned int  nBitMask = (1 << HW_GPIO_PIN_TO_BIT_OFFSET(pin));  //ȷ���������е���ֵ
...
#if 1
//�����ش�����ʽ:�����ػ����½���,����˫�ش������ߵ͵�ƽ����
//�˺���������reg_gpio_itr_set_interrupt_type_level()����ʹ��
static inline void reg_gpio_itr_set_interrupt_type( int pin, GPIO_INTERRUPT_TYPE interruptType )
{
   ...
   HW_IRQ_SAVE( Gpio, flags );
		   ...
		   {
				   REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_INT_DE) &= ~nBitMask;   //ѡ��ʹ�õ��ߴ�����ʽ
				   
				   if((interruptType==GPIO_LOW_LEVEL_INTERRUPT_TRIGGER)||(interruptType==GPIO_FALLING_EDGE_INTERRUPT_TRIGGER))
				   	{
				   			REG_GPIO_FROM_PIN (pin, REGOFFSET_GPIO_INT_EDGE) &= ~nBitMask;  //�½��ش������ߵ͵�ƽ����
				   	}
			...
			else if(interruptType==GPIO_BOTH_EDGE_INTERRUPT_TRIGGER)	//˫�ߴ���
			{
...
//added by lee  �����ж����ͣ���edge������ʽ����level������ʽ
